pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'helm-prometheus-app'
        IMAGE_TAG = "${BUILD_ID}"
    }
    ...
    stages {
        stage("Checkout") {
            steps {
                git branch: 'main', url: 'https://github.com/tailszhang0/helm-prometheus-demo.git'
            }
        }

        stage('Build Docker Image') {
            steps {
                sh 'docker build -f app/Dockerfile -t $DOCKERHUB_USER/$IMAGE_NAME:$IMAGE_TAG .'
